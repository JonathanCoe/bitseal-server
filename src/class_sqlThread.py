...
class sqlThread(threading.Thread):
    ...
    def run(self):        
        ...
        try:
            ...
            self.cur.execute(
                '''CREATE TABLE inventory (hash blob, objecttype int, streamnumber int, payload blob, expirestime integer, tag blob, receivedTime integer, UNIQUE(hash) ON CONFLICT REPLACE)''' )
            self.cur.execute(
